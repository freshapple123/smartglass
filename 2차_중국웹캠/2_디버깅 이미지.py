    ...
    while True:
        ...
        if book_box:
            ...
            if now - book_detected_time >= 3 and now >= skip_until_time:
                ...
                cv2.imwrite(os.path.join(debug_dir, f"frame_{timestamp}.jpg"), frame)
                cv2.imwrite(os.path.join(debug_dir, f"book_crop_{timestamp}.jpg"), book_crop)
                cv2.imwrite(os.path.join(debug_dir, f"left_page_{timestamp}.jpg"), left_page)
                cv2.imwrite(os.path.join(debug_dir, f"right_page_{timestamp}.jpg"), right_page)
